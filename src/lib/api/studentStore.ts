...
export const studentStore = {
  ...
  uploadProfileImage(
    studentId: string,
    file: File
  ): Promise<{ url: string; photo_url: string }> {
    const formData = new FormData();
    formData.append("file", file);
    return apiClient
      .upload<{ url: string; photo_url: string }>(
  ...
  updateStudentFee(
    studentId: string,
    feeId: string,
    data: Partial<StudentFeeCreate>
  ): Promise<StudentFee> {
    return apiClient.patch<StudentFee>(
      `/students/${studentId}/fees/${feeId}`,
      data
    );
  },
  ...
  getStudentFeeBalances(studentId: string): Promise<StudentFeeBalance[]> {
    return apiClient.get<StudentFeeBalance[]>(
      `/students/${studentId}/fee-balances`
    );
  },

  createStudentFeeBalance(
    studentId: string,
    data: StudentFeeBalanceCreate
  ): Promise<StudentFeeBalance> {
    return apiClient.post<StudentFeeBalance>(
      `/students/${studentId}/fee-balances`,
      data
    );
  },
  ...
  updateStudentFeeBalance(
    ...
    return apiClient.patch<StudentFeeBalance>(
      `/students/${studentId}/fee-balances/${balanceId}`,
      data
    );
  },

  deleteStudentFeeBalance(
    studentId: string,
    balanceId: string
  ): Promise<{ message: string }> {
    return apiClient.delete<{ message: string }>(
  ...
  getStudentPaymentAccounts(
    studentId: string
  ): Promise<{ id: string; type: string; account_number: string }[]> {
  ...
  getStudentSponsorships(studentId: string): Promise<StudentSponsorship[]> {
    return apiClient.get<StudentSponsorship[]>(
      `/students/${studentId}/sponsorships`
    );
  },
  ...
  uploadStudentDocument(
    ...
    return apiClient.post(
      `/students/${studentId}/documents?${params.toString()}`,
      {}
    );
  },
